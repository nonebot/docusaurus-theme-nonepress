...
function SearchPage(): JSX.Element {
  ...
  algoliaHelper.on(
    ...
    ({ results: { query, hits, page, nbHits, nbPages } }) => {
      if (query === "" || !(hits instanceof Array)) {
        searchResultStateDispatcher({ type: "reset", value: null });
        return;
      }
      ...
      new IntersectionObserver(
        (entries) => {
          ...
          if (isIntersecting && prevY.current > currentY) {
            searchResultStateDispatcher({ type: "advance", value: null });
          }
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      searchResultStateDispatcher({ type: "loading", value: null });
